...
class OpenAiVisionDriver implements OcrVisionDriverInterface
{
    ...
    {
        $this->apiKey = $apiKey
            ?: (string) Setting::get('ocr_openai_api_key', config('services.ocr.openai.api_key', ''));

        $this->model = $model
            ?: (string) Setting::get('ocr_openai_model', config('services.ocr.openai.model', 'gpt-4o-mini'));
    }
...
PROMPT;
        ...
        $rawResponse = $this->callOpenAiChat($prompt, $imageDataUri);
...
PROMPT;
    ...
    public function testConnection(): array
    {
        ...
        try {
            $response = Http::withToken($this->apiKey)
                ...
                ->post($this->baseUrl, [
                    ...
                    'messages' => [
                        [
                            'role' => 'user',
                            'content' => 'Jawab dengan 1 kata: "OK" jika koneksi API OpenAI berhasil.',
                        ],
                    ],
                    'max_tokens' => 10,
                ]);
            ...
            if ($response->successful()) {
                return [
                    'success' => true,
                    'message' => "Koneksi ke OpenAI ({$this->model}) berhasil terhubung!",
                    'model' => $this->model,
                ];
            }
            ...
            return [
                'success' => false,
                'message' => "Gagal terhubung ke OpenAI API: {$errorMessage}",
                'status_code' => $response->status(),
            ];
        } catch (\Throwable $e) {
            Log::error('OpenAI Test Connection error: '.$e->getMessage());

            return [
                'success' => false,
                'message' => 'Kesalahan koneksi ke OpenAI API: '.$e->getMessage(),
            ];
        }
    }

    /**
     * Memastikan API Key OpenAI sudah dikonfigurasi.
     */
    private function ensureApiKeyConfigured(): void
    {
        if (empty($this->apiKey)) {
            throw new \InvalidArgumentException('API Key OpenAI belum dikonfigurasi. Silakan isi di Pengaturan > OCR & AI Vision.');
        }
    }

    /**
     * Panggil OpenAI Chat Completions API dengan Vision.
     */
    private function callOpenAiChat(string $prompt, string $imageDataUri): string
    {
        $payload = [
            'model' => $this->model,
            'messages' => [
        ...
        try {
            ...
            if (! $response->successful()) {
                ...
                Log::error("OpenAI Vision API Error ({$response->status()}): {$response->body()}");

                throw new \RuntimeException("Gagal memproses gambar dengan OpenAI: {$errorMessage}");
            }
            ...
            if (empty($text)) {
                throw new \RuntimeException('Tidak ada respon teks dari OpenAI API.');
            }
    ...
    /**
     * Parsing JSON dari respon OpenAI.
     */
    private function parseJsonResponse(string $raw): array
    {
        ...
        if (json_last_error() !== JSON_ERROR_NONE || ! is_array($decoded)) {
            Log::warning('Gagal decode JSON dari OpenAI, raw: '.$raw);
